    ...
    for iter_folder in tqdm(iter_folders, desc="Rendering generations"):
        ...
        try:
            if env_name != 'talakat-v0':
                ...
                img = env.render(best_chromosome['content'])
                output_filename = os.path.join(renders_dir, f'gen_{iter_num:04d}_best.png')
                img.save(output_filename)
            else:
                # For talakat-v0, render returns frames for a GIF
                frames = env.render(best_chromosome['content'])
                output_filename = os.path.join(renders_dir, f'gen_{iter_num:04d}_best.gif')
                frames[0].save(
                    output_filename,
